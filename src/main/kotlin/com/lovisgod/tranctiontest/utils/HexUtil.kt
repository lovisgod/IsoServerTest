...
//    }
    ...
    fun parseHex(data: String): ByteArray {
        val src = data.replace("\n", "").replace(" ", "").uppercase(Locale.getDefault()).toCharArray()
    ...
    @JvmOverloads
     fun toHexString(data: ByteArray, aOffset: Int = 0, aLength: Int = data.size): String {
    ...
    fun convertStringToHex(str: String): String {
        return toHexString(str.toByteArray(StandardCharsets.UTF_8))
    }
}
